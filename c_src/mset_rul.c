 ...
 *
 * Build and run standalone (original console demo, unchanged):
 *     gcc -std=c99 -O2 -Wall -Wextra -o mset_rul mset_rul.c -lm
 *     ./mset_rul
 *
 * Build as a library for the visualizer module (mset_visualizer.c/.h):
 *     gcc -std=c99 -O2 -c mset_rul.c -DMSET_RUL_NO_MAIN -o mset_rul.o
 *
 * The public API (MSET, SPRT, degradation, FailureCriteria, LinearTrend,
 * RULPrediction, ...) lives in mset_rul.h so other translation units can
 * #include it and link against this file.
 *
...
double kernel_inverse_distance(const double *u, const double *v, int n,
                               double scale)
{
...
double kernel_gaussian(const double *u, const double *v, int n,
                       double bandwidth)
{
...
double kernel_bounded_angle(const double *u, const double *v, int n,
                            double unused)
{
    (void)unused;
...
/* MSET struct is declared in mset_rul.h (public, used by the visualizer). */
...
MSET *mset_create(mset_kernel_fn kernel, double kernel_param,
 ...
 */
int mset_fit(MSET *m, const double *training, int l, int n,
            int memory_size)
{
 ...
 * stream (SprtVerdict / SPRT are declared in mset_rul.h). Two hypotheses are
 * tracked simultaneously (positive and negative shift): an MSET residual
 * R = Xest - Xobs drifts negative when the parameter rises above its healthy
 * estimate and positive when it falls below, so a one-sided test would miss
 * half of all degradation modes.
 *
 ...
 */
void sprt_init(SPRT *s, double mean, double sigma,
...
void accumulated_degradation(const double *residuals, int k, int n,
                             double *out)
{
    double total = 0.0;
    for (int i = 0; i < k; ++i) {
 ...
 */
/* FailureCriteria is declared in mset_rul.h. */
int failure_criteria_init(FailureCriteria *c,
 ...
 */
/* LinearTrend is declared in mset_rul.h. */
int linear_trend_fit(LinearTrend *t, const double *times,
...
/* PowerTrend is declared in mset_rul.h. */
int power_trend_fit(PowerTrend *p, const double *times,
                    const double *values, int k)
{
...
/* RULPrediction is declared in mset_rul.h. */
...
double rng_gauss(Rng *r, double mu, double sigma)  /* Box-Muller */
{
    if (r->has_gauss) {
        r->has_gauss = 0;
 ...
 * out must hold days x DEMO_PARAMS. (DEMO_PARAMS is declared in mset_rul.h.)
 */
